...
import { hasUnsafeControlText, hasUnsafePublicText } from "../public-text-policy";
...
export function parseFreeTextMessage(body: unknown): FreeTextCommunicationMessage {
  ...
  if (hasUnsafeContactOrControlText(normalized)) {
    if (hasUnsafeControlText(normalized)) throw invalidFreeText();
    throw new GameRuleError(
...
/** Shared safety boundary for any user-entered text crossing stranger scopes. */
export function hasUnsafeContactOrControlText(value: string): boolean {
  return hasUnsafePublicText(value);
}
